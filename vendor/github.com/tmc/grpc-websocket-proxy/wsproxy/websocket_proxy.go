...
func (p *Proxy) proxy(w http.ResponseWriter, r *http.Request) {
	...
	// To work around this, we send at least a non-nil empty body so that it can be drained and the call
	...
	// Keep the pipe reader and drain it in a goroutine to prevent the write end from blocking.
	requestBodyR, requestBodyW := io.Pipe()
	go io.Copy(io.Discard, r.Body) // Drain the original request body
